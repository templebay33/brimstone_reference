...
Shadows of Brimstone does not usually require the Heroes perform and backtracking during an Adventure/Mission. Once the Heroes have explored further into the mission, previous map tiles that no longer contain any models may be cleaned up from the table. Likewise, map diagrams should be cleaned up if an AI is displaying the map tiles to follow along with an adventure.
